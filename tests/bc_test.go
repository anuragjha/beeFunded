...
func TestBlockChainBasic(t *testing.T) {
	...
	bc := bc_p.Blockchain{}
	bc.Chain = map[int32][]b_p.Block{}
	bc.Length = 0
	bc_p.DecodeFromJSON(&bc, jsonBlockChain)
	// if err != nil {
	// 	fmt.Println(err)
	// 	t.Fail()
	// }
	//jsonNew, err := bc.EncodeToJson()
	jsonNew := bc_p.EncodeToJSON(&bc)
	// if err != nil {
	// 	fmt.Println(err)
	// 	t.Fail()
	// }
	var realValue []b_p.BlockJson
	var expectedValue []b_p.BlockJson
